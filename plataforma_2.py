...
class Game:
    ...
    def __init__(self, scenario=SCENARIO):
        ...
        self.particles = Particles(self.space)
    ...
    def update(self):
            ...
            if pyxel.btn(pyxel.KEY_UP):
                for _ in range(2):
                    self.particles.emmit(
                        ...
                        velocity=self.player.rotation_vector.perpendicular(),
                    )
            self.player.update()
        ...
        if self.state is GameState.GAME_OVER and pyxel.btn(pyxel.KEY_ENTER):
            game = Game()
